...
const JoinEmployer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save username, birth, and photo in Firebase
      await updateUserProfile(name, data.data.display_url);

      // Save user with role and birth
      await saveUser({ email, name, photoURL: data.data.display_url, role: 'employer', birth });
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const userInfo = {
        ...
        role: 'employer', // Add role here
        status: 'verified'
      };
      await saveUser(userInfo); // Save user with role
      navigate(from);
      toast.success('SignUp Successful');
    } catch (error) {
      console.log(error);
  ...
  const saveUser = async (user) => {
    const currentUser = {
      ...
      birth: user?.birth // Include birth in saveUser function
    };
    ...
    <div className="flex justify-center items-center min-h-screen">
      <div className="flex flex-col p-6 rounded-md sm:p-10 bg-gray-100 text-gray-900">
        <div className="mb-8 text-center">
          <h1 className="my-3 text-4xl font-bold">Join as Employer</h1>
